...
#define qmi_rsp_check_and_return() do { \
        ...
        if (pResponse == NULL) { \
            log_info("%s get nil response", __func__); \
            return err; \
        } \
...
static ushort WdaSetDataFormat(PQMUX_MSG pMUXMsg, void *arg) {
    ...
    if (qmap_settings->rx_urb_size == 0) {
        ...
        if (sizeof(*linkProto) != 7 )
            log_info("%s sizeof(*linkProto) = %ld, is not 7!", __func__, sizeof(*linkProto) );
...
int QmiThreadSendQMITimeout(PQCQMIMSG pRequest, PQCQMIMSG *ppResponse, unsigned msecs) {
    int ret;
    extern PROFILE_T *gstProfile;
    PROFILE_T *profile = gstProfile;
    ...
    if (ret == 0) {
        ...
        if (!ret) {
            ...
            log_error("%s pthread_cond_timeout_np ret=%d", __func__, ret);
        }
    }
...
int QmiThreadSendQMI(PQCQMIMSG pRequest, PQCQMIMSG *ppResponse) {
    return QmiThreadSendQMITimeout(pRequest, ppResponse, 5 * 1000);
}
...
void QmiThreadRecvQMI(PQCQMIMSG pResponse) {
    ...
    if (s_pRequest && is_response(s_pRequest, pResponse)) {
        ...
        send_to_dial(RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED);
        ...
        send_to_dial(RIL_UNSOL_DATA_CALL_LIST_CHANGED);
    } else if ((pResponse->QMIHdr.QMIType == QMUX_TYPE_NAS)
               && (le16_to_cpu(pResponse->MUXMsg.QMUXMsgHdrResp.Type) == QMINAS_SYS_INFO_IND)) {
        send_to_dial(RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED);
    } else {
        if (verbose)
            log_info("nobody care this qmi msg!!");
    }
    pthread_mutex_unlock(&s_commandmutex);
}
...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    if (profile->qmap_iface) {
        ...
        qmap_settings.MuxId = (profile->qmap_iface != profile->iface) ? (0x80 + profile->pdp) : 0x81;

        if (profile->dri_type == DRI_MHI) { //SDX20_PCIE
            qmap_settings.rx_urb_size = 16*1024; //must same as rx_urb_size defined in GobiNet&qmi_wwan driver
        ...
        if (profile->dri_type == DRI_GOBI) //GobiNet set data format in GobiNet driver
            goto skip_WdaSetDataFormat;

        if (profile->qmap_iface != profile->iface) //QMAP MUX enabled, set data format in quectel-qmi-proxy
            goto skip_WdaSetDataFormat;
    }
...
//log_info("d1:%d, d2:%d,d3:%d",d1,d2,d3);
        if ( d1<10 && d2<10 && d3<10 ) {
            ...
            *(p_mcc+2) = '0' + d3;
            ...
            //log_info( "invalid digits %d %d %d", d1, d2, d3 );
            *p_mcc = 0;
        }
    }
}
        ...
        *p_mnc = 7;
        ...
        if ( d1<10 && d2<10 ) {
            *p_mnc = d1*10 + d2;
            ...
            //log_info( "invalid digits %d %d", d1, d2, 0 );
            *p_mnc = 0;
        }
    }
}
...
static int requestGetHomeNetwork(ushort *p_mcc, ushort *p_mnc, ushort *p_sid, ushort *p_nid) {
    ...
    if (pHomeNetwork && p_mcc && p_mnc ) {
        ...
        *p_mnc = le16_to_cpu(pHomeNetwork->MobileNetworkCode);
        //log_info("%s MobileCountryCode: %d, MobileNetworkCode: %d", __func__, *pMobileCountryCode, *pMobileNetworkCode);
    }
...
static char *wwan_data_class2str(uint class) {
    unsigned int i = 0;
...
int requestQueryDataCall(uchar  *pConnectionStatus, int curIpFamily) {
    ...
    *pConnectionStatus = QWDS_PKT_DATA_DISCONNECTED;
    ...
    if (LOOP_MSG) {
        if (oldConnectionStatus != *pConnectionStatus || verbose) {
            log_info("%s %sConnectionStatus: %s", __func__, (curIpFamily == IpFamilyV4) ? "IPv4" : "IPv6",
...
int requestSetupDataCall(PROFILE_T *profile, int curIpFamily) {
    ...
    if (curIpFamily == IpFamilyV4) {
        ...
        log_info("%s WdsConnectionIPv4Handle: 0x%08x", __func__, WdsConnectionIPv4Handle);
    } else {
...
int requestSetProfile(PROFILE_T *profile) {
    ...
    log_info("%s[%d] %s/%s/%s/%d", __func__, profile->pdp, profile->apn, profile->user, profile->password, profile->auth);
...
int requestGetProfile(PROFILE_T *profile) {
    ...
    if (pApnName/* && le16_to_cpu(pApnName->TLVLength)*/)
        apn = strndup((const char *)(&pApnName->ApnName), (size_t)le16_to_cpu(pApnName->TLVLength));
    if (pUserName/*  && pUserName->UserName*/)
        user = strndup((const char *)(&pUserName->UserName), (size_t)le16_to_cpu(pUserName->TLVLength));
    if (pPassWd/*  && le16_to_cpu(pPassWd->TLVLength)*/)
        password = strndup((const char *)(&pPassWd->Passwd), (size_t)le16_to_cpu(pPassWd->TLVLength));
...
int requestBaseBandVersion(const char **pp_reversion) {
    ...
    if (revId && le16_to_cpu(revId->TLVLength)) {
        char *DeviceRevisionID = strndup((const char *)(&revId->RevisionID), (size_t)le16_to_cpu(revId->TLVLength));
        log_info("%s %s", __func__, DeviceRevisionID);
...
int requestSetOperatingMode(uchar OperatingMode) {
    ...
    log_info("%s(%d)", __func__, OperatingMode);
